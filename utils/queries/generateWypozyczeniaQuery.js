function generateWypozyczeniaQuery(filters, numerKarty) {
  let query =
    "SELECT * FROM project.widok_wypozyczen WHERE numer_karty_bibliotecznej = $1";
  ...
  if (filters.kategorie) {
    const kategorieArray = Array.isArray(filters.kategorie)
      ? filters.kategorie
      : [filters.kategorie];
    const kategoriePlaceholders = kategorieArray
  ...
  if (filters.biblioteki) {
    const bibliotekiArray = Array.isArray(filters.biblioteki)
      ? filters.biblioteki
      : [filters.biblioteki];
    const bibliotekiPlaceholders = bibliotekiArray
      .map((_, i) => `$${params.length + i + 1}`)
      .join(", ");
